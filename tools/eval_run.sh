...
#   bash tools/eval_run.sh <wandb_run_id> [final_ckpt_basename]
#
...
if [ -z "$RUN" ]; then
    echo "usage: $0 <wandb_run_id> [final_ckpt_basename]" >&2
    exit 1
fi
...
mkdir -p "$TD"
./dyna_train --traj "$TD/100ep.bin" --load "$CKPT" --episodes 100 --seed 42
